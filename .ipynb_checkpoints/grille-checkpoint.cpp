...
Case::Case():valeur{-1}, occupe{false}, type{" "}, camp{-1}{
};
...
void Grille::poseBrindille(Coord c){
	...
	grille[c.getLig()][c.getCol()].type = "B";
}
...
void Grille::Defcamp(Coord c, int ind){
	if(not(grille[c.getLig()][c.getCol()].occupe) or (grille[c.getLig()][c.getCol()].occupe and grille[c.getLig()][c.getCol()].valeur != -1)) {
		throw invalid_argument("on ne peut pas faire changer de camp une case qui n'est pas une brindille");
	}
	grille[c.getLig()][c.getCol()].camp = ind;
	
}
		
void Grille::enleveBrindille(Coord c){
	if(grille[c.getLig()][c.getCol()].valeur == -1 and grille[c.getLig()][c.getCol()].occupe and grille[c.getLig()][c.getCol()].type == "B" ){ 
		grille[c.getLig()][c.getCol()].occupe = false;
		grille[c.getLig()][c.getCol()].type = " ";
	}else{
...
void Grille::poseTermite(Coord c, int idT, int ind){
...
void Grille::enleveTermite(Coord c){
	if(grille[c.getLig()][c.getCol()].valeur != -1 and grille[c.getLig()][c.getCol()].occupe == true){
		grille[c.getLig()][c.getCol()].valeur = -1;
		grille[c.getLig()][c.getCol()].occupe = false;
		grille[c.getLig()][c.getCol()].camp = -1;
	}else{
...
void Grille::poseNid(Coord c, int num){
	if(not(estVide(c))){
		throw invalid_argument("on ne peut pas poser de nid");
	}
	grille[c.getLig()][c.getCol()].camp = num;
	grille[c.getLig()][c.getCol()].occupe = true;
	grille[c.getLig()][c.getCol()].type = "N";
	grille[c.getLig()][c.getCol()].valeur = -1;
}
